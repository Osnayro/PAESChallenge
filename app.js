...
async function ejecutarLoginFirebase() {
    ...
    if (!email || !password) {
        mostrarErrorRegistro('Por favor ingresa tu correo y contraseña.');
        return;
    }

    try {
        mostrarErrorRegistro('Iniciando sesión en Firebase... 🔑');
        if (window.paesFirebase) {
            await window.paesFirebase.login(email, password);
            ocultarPantallaRegistro();
        } else {
            throw new Error('Servicio Firebase no inicializado');
        }
    } catch (err) {
        mostrarErrorRegistro(`Error: ${traductorErroresFirebase(err.code || err.message)}`);
    }
}
...
async function ejecutarRegistroFirebase() {
    ...
    if (!name || !email || !password) {
        mostrarErrorRegistro('Por favor completa Nombre, Email y Contraseña.');
        return;
    }
    ...
    try {
        mostrarErrorRegistro('Creando tu cuenta en Firebase... 🚀');
        if (window.paesFirebase) {
...
async function ejecutarGoogleSignIn() {
    try {
        mostrarErrorRegistro('Conectando con Google... 🚀');
        if (window.paesFirebase) {
        ...
        mostrarErrorRegistro(`Error en Google Sign-In: ${err.message}`);
    }
}
...
async function ejecutarCerrarSesion() {
    if (window.paesFirebase) {
        await window.paesFirebase.logout();
    }
...
function traductorErroresFirebase(code) {
    ...
    if (code.includes('auth/email-already-in-use')) {
        return 'Este correo ya está registrado en PAES Challenge.';
    }
        ...
        if (window.paesFirebase) {
            window.paesFirebase.onAuthChange(async (user) => {
                if (user) {
                    ...
                    if (!localName || localName === '') mostrarPantallaRegistro();
...
function startLevel(lv) {
    ...
    const sanitized = (window.paesFirebase && typeof window.paesFirebase.registerQuestions === 'function')
        ? window.paesFirebase.registerQuestions(cloned)
        : cloned;
    state.questions = lv === 1 ? sanitized : shuffleArray(sanitized);
...
function checkMultipleAnswer(oi, q) {
    ...
    const verification = (window.paesFirebase && typeof window.paesFirebase.checkAnswer === 'function')
        ? window.paesFirebase.checkAnswer(q.id, oi)
        : { isCorrect: oi === q.correct, correctAnswer: q.correct, explanation: q.explanation || '', evidenceText: q.evidenceText || null };

    const si = q._shuffledIndices;
    const cdi = si.indexOf(verification.correctAnswer);
    let cdi2 = -1;
    ...
    if (verification.isCorrect) {
        ...
        showFeedback(`¡Correcto! ${verification.explanation || q.explanation || ''}${q.isBonus?' 🎁 BONUS!':''}`, q.isBonus?'bonus':'correct');
        if (state.currentLevel === 1 && q.textKey && (verification.evidenceText || q.evidenceText)) {
            resaltarEvidenciaEnLectura(q.textKey, verification.evidenceText || q.evidenceText, 'correct');
        }
        handleCorrectAnswer(tp);
    } else {
        if (opts[cdi2]) opts[cdi2].classList.add('incorrect');
        if (opts[cdi]) opts[cdi].classList.add('correct');
        showFeedback(`Incorrecto. ${verification.explanation || q.explanation || ''}`, 'incorrect');
        if (state.currentLevel === 1 && q.textKey && (verification.evidenceText || q.evidenceText)) {
            resaltarEvidenciaEnLectura(q.textKey, verification.evidenceText || q.evidenceText, 'incorrect');
        }
...
function saveToLeaderboard() {
    ...
    if (authUser) {
        ...
        const payload = construirPayloadResultados(authUser.displayName || 'Estudiante');
        payload.uid = authUser.uid;

        // 1. Guardar intento en Firestore
        window.paesFirebase.saveAttempt(payload).catch(err => console.warn("⚠️ Error guardando intento en Firestore:", err));
        
        // 2. Actualizar estadísticas globales del usuario y leaderboard
        window.paesFirebase.updateUserStats(
            authUser.uid,
            state.score,
            state.maxStreak,
            state.badges,
            state.powerups
        ).catch(err => console.warn("⚠️ Error actualizando stats en Firestore:", err));

        // 3. Fallback / espejo local y Google Sheets
        enviarResultadosGoogleSheets(authUser.displayName || 'Estudiante');
    } else {
        // Usuario Invitado / Modo Offline
        showNamePromptModal((name) => {
            const nombreFinal = name || 'Invitado';
            safeLocalSet('paes_jugador_nombre', nombreFinal);
            if (name) {
                const lb = JSON.parse(safeLocalGet('paes_leaderboard_v4','[]'));
                lb.push({ name: nombreFinal, score:state.score, badges:Object.values(state.badges).filter(Boolean).length, tiempo:state.tiempoTotalDesafio, promedio:state.totalPreguntasRespondidas>0?(state.tiempoTotalDesafio/state.totalPreguntasRespondidas).toFixed(1):0, date:new Date().toLocaleDateString() });
...
function loadLeaderboard() {
    ...
    // Intentar suscripción en tiempo real con Firestore
        ...
        unsubscribeLeaderboard = window.paesFirebase.subscribeLeaderboard(20, (items) => {
            if (items && items.length > 0) {
                ...
                items.forEach((e, i) => {
                    ...
                    r.innerHTML = `<td class="${rankClass}">${i+1}</td><td>${medalIcon}${e.displayName || 'Estudiante'} <small style="opacity:0.7;font-size:0.75rem;">(${e.colegio || 'General'})</small></td><td>${e.highScore || 0} pts</td><td>${'🏅'.repeat(e.badgesUnlocked || 0)}</td>`;
                    tbody.appendChild(r);
                });
                return;
            }
